...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching ${sport} schedule data from Highlightly for 2025-2026 season...`);
    ...
    // Function to fetch schedule from Highlightly API
    const fetchHighlightlySchedule = async (sportType: string) => {
      ...
      try {
        ...
        const response = await fetch(`https://api.highlightly.com/v1/${endpoint}/schedule/2025-2026`, {
          headers: {
        ...
        if (!response.ok) {
          console.error(`Highlightly ${sportType} fetch failed:`, response.status, await response.text());
          return [];
        }

        const data = await response.json();
        console.log(`Received ${data.games?.length || 0} games from Highlightly for ${sportType} 2025-2026 season`);
        ...
        for (const game of data.games || []) {
          transformedGames.push({
            ...
            game_id: game.game_id || `highlightly_${sportType.toLowerCase()}_${Date.now()}_${Math.random()}`,
            sport: sportType,
        ...
        console.error(`Error fetching ${sportType} schedule from Highlightly:`, error);
        return [];
      }
    };
    ...
    if (sport === 'all') {
      ...
      for (const sportType of sports) {
        const gameData = await fetchHighlightlySchedule(sportType);
        allGames = allGames.concat(gameData);
      }
    } else {
      const gameData = await fetchHighlightlySchedule(sport.toUpperCase());
      allGames = allGames.concat(gameData);
    }

    if (allGames.length === 0) {
      console.log('No schedule data available from Highlightly for 2025-2026 season');
      return new Response(JSON.stringify({ 
        success: false, 
        message: 'No schedule data available for 2025-2026 season',
        games_updated: 0 
      }), {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    ...
    console.log(`Found ${uniqueGames.length} unique games for 2025-2026 season`);
    ...
    return new Response(JSON.stringify({ 
      success: true, 
      message: `Updated schedule with ${uniqueGames.length} games for 2025-2026 season from Highlightly`,
